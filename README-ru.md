...
> [!IMPORTANT]
> В файле `env` указывайте значения без кавычек и не добавляйте пробелы вокруг `=`. Значения не должны содержать символы `\`, `"` или `'`. Надёжный IPsec PSK должен состоять как минимум из 20 случайных символов.
